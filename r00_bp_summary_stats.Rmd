---
title: "Honduras R00 BP Summary Stats"
author: "Ethan Walker"
date: "7 Oct 2019"
output: pdf_document
---
...
r00_data_sphygmo_p2 <- r00_full_long %>%
  ...
  filter(!is.na(sys_bp_periph))  # 955 obs from 184 participants  
```

```{r, fig.height=4, fig.width=8}
sbp_boxplot <- r00_full_long %>% 
  select(phase, study_arm, sys_bp_periph, assigned_stove, stove_use_3) %>% 
  ...
  filter(!is.na(sys_bp_periph)) %>% 
  ggplot(aes(1, sys_bp_periph)) +
    geom_boxplot(aes(assigned_stove, sys_bp_periph, fill = assigned_stove, size = 1), 
    ...
    ylab(expression(paste("Systolic Blood Pressure (mmHg)"))) +
...
ggsave("sbp_boxplots.jpeg", height = 4, width = 8)
```

# Eval missing data for manuscript flow chart
```{r}
n_check <- r00_model_data_outliers_removed %>% 
  filter(!is.na(pulse_pressure_central)) %>% 
  filter(phase == 6) %>% 
  #filter(!is.na(assigned_stove)) %>% 
  #filter(!is.na(log_p_twa)) %>% 
  filter(!is.na(log_a_twa)) %>% 
  group_by(study_arm) %>% 
  count()
```
...
bp_sd <- r00_data %>% 
  select(study_arm, sys_bp_periph, house_id, dia_bp_periph) %>% 
  filter(!is.na(study_arm)) %>% 
  filter(!is.na(sys_bp_periph)) %>% 
  #filter(phase == 1) %>% 
  summarize("mean sbp" = mean(sys_bp_periph), "sd sbp" = sd(sys_bp_periph),
            "mean dbp" = mean(dia_bp_periph), "sd dbp" = sd(dia_bp_periph)) 
...
bp_anova_data <- r00_data %>% 
  select(study_arm, sys_bp_periph, house_id, dia_bp_periph, assigned_stove) %>% 
  filter(assigned_stove == "Traditional") %>% 
  filter(!is.na(study_arm)) %>% 
  filter(!is.na(sys_bp_periph))

bp_anova_data5 <- r00_data_5phases %>% 
  select(study_arm, sys_bp_periph, house_id, dia_bp_periph) %>% 
  filter(!is.na(study_arm)) %>% 
  filter(!is.na(sys_bp_periph)) 

bp_anova_data6 <- r00_data_6phases %>% 
  select(study_arm, sys_bp_periph, house_id, dia_bp_periph) %>% 
  filter(!is.na(study_arm)) %>% 
  filter(!is.na(sys_bp_periph)) 

sbp_anova <- aov(sys_bp_periph ~ house_id, data = bp_anova_data)
dbp_anova <- aov(dia_bp_periph ~ house_id, data = bp_anova_data)
summary(sbp_anova)
summary(dbp_anova)

# pre-intervention data only
sqrt(62.3) #7.9 for SBP
sqrt(22.83) #4.8 for DBP


# Hand-calc confirming anova calculation above
bp_sd_within_person <- r00_data %>% 
  select(study_arm, sys_bp_periph, house_id, dia_bp_periph) %>% 
  filter(!is.na(study_arm)) %>% 
  filter(!is.na(sys_bp_periph)) %>% 
  #filter(phase == 1) %>% 
  group_by(house_id) %>% 
  summarize("meansbp" = mean(sys_bp_periph), "sdsbp" = sd(sys_bp_periph), 
            "varsbp" = var(sys_bp_periph)) %>% 
  summarize(sqrt(mean(varsbp, na.rm = TRUE)))
```

# Look at differences in outcomes by who made measurement
```{r}
health_admin <- r00_data %>% 
  ungroup() %>% 
  select(house_id, phase, admin_health, sys_bp_periph, dia_bp_periph,
  ...
  mutate(admin = if_else(phase == 3 & admin == 5, 3, admin),
         ...
         health_admin_final = factor(admin, levels = c(1,2,3,4,5), 
                              labels = c("Bonnie", "Sarah", "Laura", 
                                         "Mariel", "Gloribel"))) %>% 
  group_by(health_admin_final) %>% 
  summarise("n" = n(),"Mean sys BP" = mean(sys_bp_periph, na.rm = TRUE), 
            "Mean Dia BP" = mean(dia_bp_periph, na.rm = TRUE), 
            "Mean AIx" = mean(aug_index, na.rm = TRUE), 
            "Mean CPP" = mean(pulse_pressure_central, na.rm = TRUE))

kable(health_admin, digits = 1)
```
...
# Variables grouped by assigned stove
```{r}
summary_stats <- r00_data %>% 
  filter(!is.na(sys_bp_periph)) %>% 
  select(assigned_stove, "Augmentation Index" = aug_index, "Heart Rate" = hr, 
         "Systolic BP Peripheral" = sys_bp_periph, "Diastolic BP Peripheral" = dia_bp_periph, 
         "MAP Peripheral" = map_periph, "Systolic BP Central" = sys_bp_central, 
         "Diastolic BP Central" = dia_bp_central, "MAP Central" = map_central, 
         "Augmentation Pressure" = aug_pressure, "Augmentation Index 75" = aug_index_75,
  ...
  summarise_all(funs("_mean" = mean, "_sd" = sd, "_median" = median, 
                     "_min" = min, "_max" = max, "_n" = n())) %>% 
  gather(var, value, -assigned_stove) %>% 
  separate(var, into = c("var", "stat"), sep = "__") %>% 
  spread(stat, value) %>% 
  arrange(var, assigned_stove) %>% 
  select(var, assigned_stove, n, mean, sd, min, median, max) %>% 
  rename("Variable" = var, "Assigned Stove" = assigned_stove) 
kable(summary_stats, digits = 2)
```
...
```{r, eval=FALSE}
# SBP grouped by stove use and study arm - all phases
## Grouped by stove use

summary_sbp_stove_use <- r00_data %>% 
  filter(!is.na(assigned_stove)) %>% 
  filter(!is.na(sys_bp_periph)) %>% 
  select(stove_use_3, assigned_stove, sys_bp_periph, phase, study_arm,
         a_twa, p_twa, age_cat_40, waist_cat, ses_cat_2, salt) %>% 
  mutate(salt = as.numeric(salt)) %>% 
  group_by(stove_use_3, phase) %>% 
  summarise("Data" = "all",
            "Mean SBP" = mean(sys_bp_periph, na.rm = TRUE),
            "SD" = sd(sys_bp_periph), 
            "n" = n(),
            "%" = n()/1168*100,
            "25th" = quantile(sys_bp_periph, c(.25)),
            "75th" = quantile(sys_bp_periph, c(.75)),
            "Median" = median(sys_bp_periph),
            "Min" = min(sys_bp_periph), 
            "Max" = max(sys_bp_periph),
            "IQR" = IQR(sys_bp_periph))
kable(summary_sbp_stove_use, digits = 1)

summary_sbp_stove_use_6phases <- r00_data_6phases %>% 
  filter(!is.na(assigned_stove)) %>% 
  filter(!is.na(sys_bp_periph)) %>% 
  select(stove_use_3, assigned_stove, sys_bp_periph, phase, study_arm,
         a_twa, p_twa, age_cat_40, waist_cat, ses_cat_2, salt) %>% 
  ...
  summarise("Data" = "6 phases",
            ...
            "Max" = max(sys_bp_periph),
            "IQR" = IQR(sys_bp_periph))
kable(summary_sbp_stove_use_6phases, digits = 1)

summary_sbp_stove_use_5phases <- r00_data_5phases %>% 
  filter(!is.na(assigned_stove)) %>% 
  filter(!is.na(sys_bp_periph)) %>% 
  select(stove_use_3, assigned_stove, sys_bp_periph, phase, study_arm,
         a_twa, p_twa, age_cat_40, waist_cat, ses_cat_2, salt) %>% 
  ...
  summarise("Data" = "5 phases",
            ...
            "Max" = max(sys_bp_periph),
            "IQR" = IQR(sys_bp_periph))
kable(summary_sbp_stove_use_5phases, digits = 1)

sbp_summary_all_stove_use <- rbind(summary_sbp_stove_use, summary_sbp_stove_use_6phases,
                     summary_sbp_stove_use_5phases) 
kable(sbp_summary_all_stove_use, digits = 1)


## Grouped by study arm

summary_sbp_study_arm <- r00_data %>% 
  filter(!is.na(assigned_stove)) %>% 
  filter(!is.na(sys_bp_periph)) %>% 
  select(stove_use_3, assigned_stove, sys_bp_periph, phase, study_arm,
         a_twa, p_twa, age_cat_40, waist_cat, ses_cat_2, salt) %>% 
  ...
  summarise("Data" = "all",
            ...
            "Max" = max(sys_bp_periph),
            "IQR" = IQR(sys_bp_periph))
kable(summary_sbp_study_arm, digits = 1)

summary_sbp_study_arm_6phases <- r00_data_6phases %>% 
  filter(!is.na(assigned_stove)) %>% 
  filter(!is.na(sys_bp_periph)) %>% 
  select(stove_use_3, assigned_stove, sys_bp_periph, phase, study_arm,
         a_twa, p_twa, age_cat_40, waist_cat, ses_cat_2, salt) %>% 
  ...
  summarise("Data" = "6 Phases",
            ...
            "Max" = max(sys_bp_periph),
            "IQR" = IQR(sys_bp_periph))
kable(summary_sbp_study_arm_6phases, digits = 1)

summary_sbp_study_arm_5phases <- r00_data_5phases %>% 
  filter(!is.na(assigned_stove)) %>% 
  filter(!is.na(sys_bp_periph)) %>% 
  select(stove_use_3, assigned_stove, sys_bp_periph, phase, study_arm,
         a_twa, p_twa, age_cat_40, waist_cat, ses_cat_2, salt) %>% 
  ...
  summarise("Data" = "5 Phases",
            ...
            "Max" = max(sys_bp_periph),
            "IQR" = IQR(sys_bp_periph))
kable(summary_sbp_study_arm_5phases, digits = 1)

sbp_summary_all_study_arm <- rbind(summary_sbp_study_arm, summary_sbp_study_arm_6phases,
                     summary_sbp_study_arm_5phases) 
kable(sbp_summary_all_study_arm, digits = 1)


## Ungrouped

summary_sbp <- r00_data %>% 
  filter(!is.na(assigned_stove)) %>% 
  filter(!is.na(sys_bp_periph)) %>% 
  select(stove_use_3, assigned_stove, sys_bp_periph, phase, study_arm,
         a_twa, p_twa, age_cat_40, waist_cat, ses_cat_2, salt) %>% 
  ...
  summarise("Data" = "all",
            ...
            "Max" = max(sys_bp_periph),
            "IQR" = IQR(sys_bp_periph))
kable(summary_sbp, digits = 1)

summary_sbp_6phases <- r00_data_6phases %>% 
  filter(!is.na(assigned_stove)) %>% 
  filter(!is.na(sys_bp_periph)) %>% 
  select(stove_use_3, assigned_stove, sys_bp_periph, phase, study_arm,
         a_twa, p_twa, age_cat_40, waist_cat, ses_cat_2, salt) %>% 
  ...
  summarise("Data" = "6 Phases",
            ...
            "Max" = max(sys_bp_periph),
            "IQR" = IQR(sys_bp_periph))
kable(summary_sbp_6phases, digits = 1)

summary_sbp_5phases <- r00_data_5phases %>% 
  filter(!is.na(assigned_stove)) %>% 
  filter(!is.na(sys_bp_periph)) %>% 
  select(stove_use_3, assigned_stove, sys_bp_periph, phase, study_arm,
         a_twa, p_twa, age_cat_40, waist_cat, ses_cat_2, salt) %>% 
  ...
  summarise("Data" = "5 Phases",
            ...
            "Max" = max(sys_bp_periph),
            "IQR" = IQR(sys_bp_periph))
kable(summary_sbp_5phases, digits = 1)

sbp_summary_all <- rbind(summary_sbp, summary_sbp_6phases,
                     summary_sbp_5phases) 
kable(sbp_summary_all, digits = 1)
```


```{r, eval=FALSE}
# DBP grouped by stove use and study arm - all phases
## Grouped by stove use

summary_dbp_stove_use <- r00_data %>% 
  filter(!is.na(assigned_stove)) %>% 
  filter(!is.na(dia_bp_periph)) %>% 
  select(stove_use_3, assigned_stove, dia_bp_periph, phase, study_arm,
         a_twa, p_twa, age_cat_40, waist_cat, ses_cat_2, salt) %>% 
  mutate(salt = as.numeric(salt)) %>% 
  group_by(stove_use_3, phase) %>% 
  summarise("Data" = "all",
            "Mean SBP" = mean(dia_bp_periph, na.rm = TRUE),
            "SD" = sd(dia_bp_periph), 
            "n" = n(),
            "%" = n()/1168*100,
            "25th" = quantile(dia_bp_periph, c(.25)),
            "75th" = quantile(dia_bp_periph, c(.75)),
            "Median" = median(dia_bp_periph),
            "Min" = min(dia_bp_periph), 
            "Max" = max(dia_bp_periph),
            "IQR" = IQR(dia_bp_periph))
kable(summary_dbp_stove_use, digits = 1)

summary_dbp_stove_use_6phases <- r00_data_6phases %>% 
  filter(!is.na(assigned_stove)) %>% 
  filter(!is.na(dia_bp_periph)) %>% 
  select(stove_use_3, assigned_stove, dia_bp_periph, phase, study_arm,
         a_twa, p_twa, age_cat_40, waist_cat, ses_cat_2, salt) %>% 
  ...
  summarise("Data" = "6 phases",
            ...
            "Max" = max(dia_bp_periph),
            "IQR" = IQR(dia_bp_periph))
kable(summary_dbp_stove_use_6phases, digits = 1)

summary_dbp_stove_use_5phases <- r00_data_5phases %>% 
  filter(!is.na(assigned_stove)) %>% 
  filter(!is.na(dia_bp_periph)) %>% 
  select(stove_use_3, assigned_stove, dia_bp_periph, phase, study_arm,
         a_twa, p_twa, age_cat_40, waist_cat, ses_cat_2, salt) %>% 
  ...
  summarise("Data" = "5 phases",
            ...
            "Max" = max(dia_bp_periph),
            "IQR" = IQR(dia_bp_periph))
kable(summary_dbp_stove_use_5phases, digits = 1)

dbp_summary_all_stove_use <- rbind(summary_dbp_stove_use, summary_dbp_stove_use_6phases,
                     summary_dbp_stove_use_5phases) 
kable(dbp_summary_all_stove_use, digits = 1)


## Grouped by study arm

summary_dbp_study_arm <- r00_data %>% 
  filter(!is.na(assigned_stove)) %>% 
  filter(!is.na(dia_bp_periph)) %>% 
  select(stove_use_3, assigned_stove, dia_bp_periph, phase, study_arm,
         a_twa, p_twa, age_cat_40, waist_cat, ses_cat_2, salt) %>% 
  ...
  summarise("Data" = "all",
            ...
            "Max" = max(dia_bp_periph),
            "IQR" = IQR(dia_bp_periph))
kable(summary_dbp_study_arm, digits = 1)

summary_dbp_study_arm_6phases <- r00_data_6phases %>% 
  filter(!is.na(assigned_stove)) %>% 
  filter(!is.na(dia_bp_periph)) %>% 
  select(stove_use_3, assigned_stove, dia_bp_periph, phase, study_arm,
         a_twa, p_twa, age_cat_40, waist_cat, ses_cat_2, salt) %>% 
  ...
  summarise("Data" = "6 Phases",
            ...
            "Max" = max(dia_bp_periph),
            "IQR" = IQR(dia_bp_periph))
kable(summary_dbp_study_arm_6phases, digits = 1)

summary_dbp_study_arm_5phases <- r00_data_5phases %>% 
  filter(!is.na(assigned_stove)) %>% 
  filter(!is.na(dia_bp_periph)) %>% 
  select(stove_use_3, assigned_stove, dia_bp_periph, phase, study_arm,
         a_twa, p_twa, age_cat_40, waist_cat, ses_cat_2, salt) %>% 
  ...
  summarise("Data" = "5 Phases",
            ...
            "Max" = max(dia_bp_periph),
            "IQR" = IQR(dia_bp_periph))
kable(summary_dbp_study_arm_5phases, digits = 1)

dbp_summary_all_study_arm <- rbind(summary_dbp_study_arm, summary_dbp_study_arm_6phases,
                     summary_dbp_study_arm_5phases) 
kable(dbp_summary_all_study_arm, digits = 1)


## Ungrouped

summary_dbp <- r00_data %>% 
  filter(!is.na(assigned_stove)) %>% 
  filter(!is.na(dia_bp_periph)) %>% 
  select(stove_use_3, assigned_stove, dia_bp_periph, phase, study_arm,
         a_twa, p_twa, age_cat_40, waist_cat, ses_cat_2, salt) %>% 
  ...
  summarise("Data" = "all",
            ...
            "Max" = max(dia_bp_periph),
            "IQR" = IQR(dia_bp_periph))
kable(summary_dbp, digits = 1)

summary_dbp_6phases <- r00_data_6phases %>% 
  filter(!is.na(assigned_stove)) %>% 
  filter(!is.na(dia_bp_periph)) %>% 
  select(stove_use_3, assigned_stove, dia_bp_periph, phase, study_arm,
         a_twa, p_twa, age_cat_40, waist_cat, ses_cat_2, salt) %>% 
  ...
  summarise("Data" = "6 Phases",
            ...
            "Max" = max(dia_bp_periph),
            "IQR" = IQR(dia_bp_periph))
kable(summary_dbp_6phases, digits = 1)

summary_dbp_5phases <- r00_data_5phases %>% 
  filter(!is.na(assigned_stove)) %>% 
  filter(!is.na(dia_bp_periph)) %>% 
  select(stove_use_3, assigned_stove, dia_bp_periph, phase, study_arm,
         a_twa, p_twa, age_cat_40, waist_cat, ses_cat_2, salt) %>% 
  ...
  summarise("Data" = "5 Phases",
            ...
            "Max" = max(dia_bp_periph),
            "IQR" = IQR(dia_bp_periph))
kable(summary_dbp_5phases, digits = 1)

dbp_summary_all <- rbind(summary_dbp, summary_dbp_6phases,
                     summary_dbp_5phases) 
kable(dbp_summary_all, digits = 1)
```
...
```{r}
summary_stats_study_arm <- r00_data %>%
  select(phase, study_arm, "Augmentation Index" = aug_index, "Heart Rate" = hr, 
         "Systolic BP Peripheral" = sys_bp_periph, "Diastolic BP Peripheral" = dia_bp_periph, 
         "MAP Peripheral" = map_periph, "Systolic BP Central" = sys_bp_central, 
         "Diastolic BP Central" = dia_bp_central, "MAP Central" = map_central, 
         ...
         "Pulse Pressure Peripheral" = pulse_pressure_periph, 
         "Pulse Pressure Central" = pulse_pressure_central) %>% 
  na.exclude("Systolic BP Peripheral") %>% 
  filter(phase == 3 | phase == 4) %>% 
  select(-phase) %>% 
  group_by(study_arm) %>% 
  summarise_all(funs("_mean" = mean, "_sd" = sd, "_median" = median, 
...
summary_stats_age_cat <- r00_data %>% 
...
summary_stats_bmi_cat <- r00_data %>% 
  select(phase, study_arm, age_baseline, bmi, "Augmentation Index" = aug_index, 
         "Heart Rate" = hr, 
         "Systolic BP Peripheral" = sys_bp_periph, "Diastolic BP Peripheral" = dia_bp_periph, 
         "MAP Peripheral" = map_periph, "Systolic BP Central" = sys_bp_central, 
         "Diastolic BP Central" = dia_bp_central, "MAP Central" = map_central, 
...
bp_cor <- round(cor(summary_correlation, method = c("spearman"), use = "complete.obs"), 2)
bp_cor
```
...
sphygmocor_boxplot <- r00_data %>% 
  select(phase, study_arm, aug_index, hr, sys_bp_periph, dia_bp_periph,
         sys_bp_central, dia_bp_central, aug_index_75,
         pulse_pressure_periph, pulse_pressure_central) %>%
  na.exclude(sys_bp_periph) %>% 
  mutate(study_arm = factor(study_arm, levels = c(1, 2), labels = c("Arm 1", "Arm 2"))) %>% 
  ggplot(aes(phase, sys_bp_periph)) +
  geom_boxplot(aes(phase, sys_bp_periph)) +
  geom_text(data = sphygmocor_means, aes(phase, `mean(sys_bp_periph)`, 
                                    label = round(`mean(sys_bp_periph)`, digits = 1)),
                                    size = 4, vjust = -2.5, color = "red") +
  geom_line(data = sphygmocor_means, aes(phase, `mean(sys_bp_periph)`, group = study_arm), 
            color = "red") +
  geom_point(data = sphygmocor_means, aes(phase, `mean(sys_bp_periph)`), 
            size = 2, color = "red") +
  ggtitle("SBP by Phase and Study Arm", subtitle = "means in red") +
...
sphygmocor_boxplot <- r00_data %>% 
  select(phase, study_arm, aug_index, hr, sys_bp_periph, dia_bp_periph,
         sys_bp_central, dia_bp_central, aug_index_75,
         pulse_pressure_periph, pulse_pressure_central) %>%
  na.exclude(dia_bp_periph) %>% 
  mutate(study_arm = factor(study_arm, levels = c(1, 2), labels = c("Arm 1", "Arm 2"))) %>% 
  ggplot(aes(phase, dia_bp_periph)) +
  geom_boxplot(aes(phase, dia_bp_periph)) +
  geom_text(data = sphygmocor_means, aes(phase, `mean(dia_bp_periph)`, 
                                    label = round(`mean(dia_bp_periph)`, digits = 1)),
                                    size = 4, vjust = -2.5, color = "red") +
  geom_line(data = sphygmocor_means, aes(phase, `mean(dia_bp_periph)`, group = study_arm), 
            color = "red") +
  geom_point(data = sphygmocor_means, aes(phase, `mean(dia_bp_periph)`), 
            size = 2, color = "red") +
  ggtitle("DBP by Phase and Study Arm", subtitle = "means in red") +
  facet_wrap(~study_arm) +
  xlab("Study Phase") +
  ylab("DBP (mmHg)") +
  theme_bw()
...
phases_completed_sphygmocor <- r00_data %>%
  select(phase, study_arm, sys_bp_periph) %>% 
  mutate(study_arm = factor(study_arm, levels = c(1, 2), labels = c("Arm 1", "Arm 2"))) %>%
  na.exclude(sys_bp_periph) %>% 
  group_by(phase, study_arm) %>%
  summarise(number = n()) %>%
...
```{r}
all_6_phases <- r00_data %>% 
  filter(!is.na(assigned_stove)) %>% 
  select(house_id, phase, study_arm, sys_bp_periph) %>% 
  mutate(study_arm = factor(study_arm, levels = c(1, 2), labels = c("Arm 1", "Arm 2"))) %>%
  filter(!is.na(sys_bp_periph)) %>% 
...
# 6 instances where SBP value entered in field doesn't match the value pulled from database
```
...
# 8 instances where DBP value entered in field doesn't match the value pulled from database
```
...
```{r}
# SBP
sbp_histogram <- r00_data %>%
  select(phase, study_arm, sys_bp_periph) %>% 
  mutate(study_arm = factor(study_arm, levels = c(1, 2), labels = c("Arm 1", "Arm 2"))) %>%
  na.exclude(sys_bp_periph) %>% 
    ggplot()+
    geom_histogram(aes(x = sys_bp_periph), bins = 30) +
...
```{r}
# DBP
dbp_histogram <- r00_data %>%
  select(phase, study_arm, dia_bp_periph) %>% 
  mutate(study_arm = factor(study_arm, levels = c(1, 2), labels = c("Arm 1", "Arm 2"))) %>%
  na.exclude(dia_bp_periph) %>% 
    ggplot()+
    geom_histogram(aes(x = dia_bp_periph), bins = 30) +
    facet_wrap(~phase, ncol = 2) +
    theme_bw() +
    scale_fill_manual(values=c("#33CC99",
                               "#339999",
...
# SBP
line_plots <- r00_data %>% 
  select(house_id, phase, study_arm, sys_bp_periph, dia_bp_periph) %>% 
  mutate(study_arm = factor(study_arm, levels = c(1, 2), labels = c("Arm 1", "Arm 2"))) %>%
  na.exclude(sys_bp_periph) %>% 
  arrange(house_id, phase) %>% 
  group_by(house_id) %>% 
  mutate(increase = if_else(sys_bp_periph < lead(sys_bp_periph), "TRUE", "FALSE")) %>% 
  ggplot() +
  geom_line(aes(phase, sys_bp_periph, color = increase, group = house_id)) +
  theme_bw() +
  ggtitle("SBP Line plots by participant") +
...
line_plots <- r00_data %>% 
  ...
  arrange(house_id, phase) %>% 
  group_by(house_id) %>% 
  mutate(increase = if_else(dia_bp_periph < lead(dia_bp_periph), "TRUE", "FALSE")) %>% 
  ggplot() +
  geom_line(aes(phase, dia_bp_periph, color = increase, group = house_id)) +
  theme_bw() +
  ggtitle("DBP Line plots by participant") +
  theme(legend.position = "none") +
  facet_wrap(~study_arm)
line_plots
```