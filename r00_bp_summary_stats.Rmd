...
r00_data_6phases <- r00_full_long %>%
  ...
  mutate(phase_complete = if_else(!is.na(sys_bp_periph), 1, 0)) %>% 
...
```{r}
summary_stats_stove <- r00_data %>% 
  filter(!is.na(assigned_stove)) %>% 
  filter(!is.na(sys_bp_periph)) %>% 
  select(assigned_stove, sys_bp_periph, dia_bp_periph) %>% 
  group_by(assigned_stove) %>% 
  summarize(n(), mean(sys_bp_periph), sd(sys_bp_periph), mean(dia_bp_periph),
            sd(dia_bp_periph))
summary_stats_stove
```
...
summary_sbp_6phases <- r00_data_6phases %>% 
  ...
  summarise("Data" = "6 Phases",
            ...
            "Max" = max(sys_bp_periph),
            "IQR" = IQR(sys_bp_periph))
kable(summary_sbp_phase2_no, digits = 1)
...
summary_dbp_6phases <- r00_data_6phases %>% 
  ...
  summarise("Data" = "6 Phases",
            ...
            "Max" = max(dia_bp_periph),
            "IQR" = IQR(dia_bp_periph))
kable(summary_dbp_phase2_no, digits = 1)
...
sphygmocor_boxplot <- r00_data %>% 
  ...
  geom_point(data = sphygmocor_means, aes(phase, `mean(sys_bp_periph)`), 
            size = 2, color = "red") +
  ggtitle("SBP by Phase and Study Arm", 
...
```{r}
# HR
hr_histogram <- data_outliers_removed %>%
...
pulse_pressure_peripheral_histogram <- data_outliers_removed %>%
...
# sys_bp_central
sys_bp_central_histogram <- data_outliers_removed %>%
...
# dia_bp_central
dia_bp_central_histogram <- data_outliers_removed %>%
