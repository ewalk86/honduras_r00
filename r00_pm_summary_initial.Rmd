...
# Area PM - total
```{r}
summary_area_pm_study_arm <- r00_data_6phases %>% 
...
# Personal PM - total
```{r}
summary_personal_pm_study_arm <- r00_data_6phases %>% 
...
```{r}
summary_area_pm_stove <- r00_data_6phases %>% 
...
```{r}
summary_personal_pm_stove <- r00_data_6phases %>% 
...
```{r}
summary_area_pm_stove_use <- r00_data_6phases %>% 
...
summary_area_pm_stove_use <- r00_data_6phases %>% 
  select(stove_use_days_stack, study_arm, a_twa) %>% 
  filter(!is.na(study_arm)) %>% 
  filter(!is.na(a_twa)) %>% 
  group_by(stove_use_days_stack) %>% 
  summarise("Mean" = mean(a_twa),
            "SD" = sd(a_twa), 
            "n" = n(),
            "25th" = quantile(a_twa, c(.25)),
...
```{r}
summary_personal_pm_stove_use <- r00_data_6phases %>% 
...
summary_personal_pm_stove_use <- r00_data_6phases %>% 
  select(stove_use_days_stack, study_arm, p_twa) %>% 
  filter(!is.na(study_arm)) %>% 
  filter(!is.na(p_twa)) %>% 
  group_by(stove_use_days_stack) %>% 
  summarise("Mean" = mean(p_twa),
