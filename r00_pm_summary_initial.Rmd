...
season_area_pm_boxplot <- r00_full_repeated_by_phase %>% 
  select(assigned_stove, study_arm, a_twa, season) %>% 
  filter(!is.na(study_arm)) %>% 
  filter(!is.na(a_twa)) %>% 
  filter(!is.na(season)) %>%
  ggplot(aes(assigned_stove, a_twa)) +
    geom_boxplot(aes(size = 1, fill = season), lwd = 1.2, colour = "black", 
               fatten = 1, outlier.size = 1.5, width = 0.35) +
    #stat_summary(aes(group = season), fun.y=mean, geom="point", shape=17, size=3, color="red") +
    theme_minimal() +
    ylim(0, 1000) +
    #geom_hline(yintercept = 50, color = "red", size = 1.2) +
    #labs(y = "Kitchen fine particulate matter (ug/m3)") +
...
season_personal_pm_boxplot <- r00_full_repeated_by_phase %>% 
  select(assigned_stove, study_arm, p_twa, season) %>% 
  filter(!is.na(study_arm)) %>% 
  filter(!is.na(p_twa)) %>% 
  filter(!is.na(season)) %>%
