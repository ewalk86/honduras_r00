...
model_temp <- lmer(aug_index ~ assigned_stove + ns(date_sphygmo, df=6) + 
                     log(ambient_bc) + temp_c +
                     (1 | house_id), r00_model_data)
#summary(model_temp)
tidy_model_temp <- tidy(model_temp, conf.int = TRUE) %>% 
    filter(grepl('assigned_stoveJusta', term)) %>% 
    mutate(model = if_else(term == "assigned_stoveJusta", "temp", "na"))
...
# Try: temp_c, temp_rolling_24, temp_max_previous, temp_max, kitchen_temp,
# ambient_pm, ambient_bc, ambient_oc
model_temp <- lmer(pulse_pressure_central ~ assigned_stove + 
                     log(ambient_bc) + temp_c +
