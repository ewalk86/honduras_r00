...
############################################################
##### AIx ITT model - participants who missed a phase
...
tidy_model_5phases <- tidy(model_5phases, conf.int = TRUE) %>% 
    filter(grepl('assigned_stoveJusta', term)) %>% 
    mutate(model = if_else(term == "assigned_stoveJusta", "missed phase", "na")) 

############################################################
##### AIx ITT model - participants who missed phase 2 - sphygmo malfunction filtered out
...
tidy_model_p2_sphygmo <- tidy(model_p2_sphygmo, conf.int = TRUE) %>% 
    filter(grepl('assigned_stoveJusta', term)) %>% 
    mutate(model = if_else(term == "assigned_stoveJusta", "missed phase 2", "na")) 
...
plot_estimates <- aix_results %>%
  mutate(model = factor(model, levels = c("all data", "complete case", "missed phase",
                                          "missed phase 2", "no bp meds", "phase", 
                                          "season", "spline 12 df"))) %>% 
  ggplot() +
...
############################################################
##### CPP ITT model - participants who missed a phase
...
############################################################
##### CPP ITT model - participants who missed phase 2 - sphygmo malfunction filtered out
