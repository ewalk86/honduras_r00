...
r00_model_data_nophase2 <- r00_model_data %>% 
  filter(phase != 2)
...
##### AIx ITT model - adjust for baseline (phase 1)
model_baseline <- lmer(aug_index ~ assigned_stove + ns(date_sphygmo, df=6) + 
                         baseline_aix +
                         (1 | house_id), r00_model_data_aix_baseline)
#summary(model_baseline)
tidy_model_baseline <- tidy(model_baseline, conf.int = TRUE) %>% 
    filter(grepl('assigned_stoveJusta', term)) %>% 
    mutate(model = if_else(term == "assigned_stoveJusta", "baseline", "na")) 
...
model_nophase2 <- lmer(aug_index ~ assigned_stove + ns(date_sphygmo, df=6) +
                      (1 | house_id), r00_model_data_nophase2)
# summary(model_nophase2)
tidy_model_nophase2 <- tidy(model_nophase2, conf.int = TRUE) %>% 
    filter(grepl('assigned_stoveJusta', term)) %>% 
    mutate(model = if_else(term == "assigned_stoveJusta", "no phase 2", "na")) 
...
tidy_model_phase2 <- tidy(model_phase2, conf.int = TRUE) %>% 
    filter(grepl('assigned_stoveJusta', term)) %>% 
    mutate(model = if_else(term == "assigned_stoveJusta", "phase 2", "na")) 


##### AIx ITT model - including term for pregnant previous phase
model_pregnant <- lmer(aug_index ~ assigned_stove + ns(date_sphygmo, df=6) +
                  pregnant_previous + (1 | house_id), r00_model_data)
# summary(model_pregnant)
tidy_model_pregnant <- tidy(model_pregnant, conf.int = TRUE) %>% 
    filter(grepl('assigned_stoveJusta', term)) %>% 
    mutate(model = if_else(term == "assigned_stoveJusta", "pregnant prev", "na")) 
...
plot_estimates <- aix_results %>%
  mutate(model = factor(model, levels = c("primary", "no bp meds", "pregnant prev",
                                          "no phase 2", "baseline", "phase 2",
...
model_nophase2 <- lmer(pulse_pressure_central ~ assigned_stove + ns(date_sphygmo, df=6) +
                      (1 | house_id), r00_model_data_nophase2)
# summary(model_nophase2)
tidy_model_nophase2 <- tidy(model_nophase2, conf.int = TRUE) %>% 
    filter(grepl('assigned_stoveJusta', term)) %>% 
...
plot_estimates <- cpp_results %>%
  mutate(model = factor(model, levels = c("primary", "no bp meds", "no phase 2", "phase 2",
                                          "6 phases", "phase", "season", "spline 12 df"))) %>% 
  ggplot() +
  geom_point(aes(x=model, y=estimate), size = 4) +
  #scale_shape_manual(values = c(15, 16, 17, 18, 13, 9)) +
