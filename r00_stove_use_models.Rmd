...
# Filtering out phase 2
r00_model_nophase2 <- stove_use_data %>% 
  filter(phase != 2)
...
r00_model_data_aix_baseline <- stove_use_data %>% 
...
##### AIx and stove_use_3 - pregnant previous phase

model_pregnant <- lmer(aug_index ~ stove_use_3 + pregnant_previous +
                        ns(date_sphygmo, df=6) + age_baseline +
                        waist_cm + school_bi + (1 | house_id), stove_use_data)
#summary(model_pregnant)
tidy_model_pregnant <- tidy(model_pregnant, conf.int = TRUE) %>% 
    filter(grepl('stove_use_3', term)) %>% 
    mutate(model = "pregnant prev") 
...
##### AIx and stove_use_3 - adjust for baseline (phase 1)
model_baseline <- lmer(aug_index ~ stove_use_3 + baseline_aix +
                             ns(date_sphygmo, df=6) + age_baseline +
                             waist_cm + school_bi + (1 | house_id), 
...
##### AIx and stove_use_3 no phase 2 data 
...
plot_estimates <- aix_results %>%
  mutate(model = factor(model, levels = c("stove use", "pregnant prev", "no phase 2", "baseline",
