...
r00_model_data_phase2_nopreg <- r00_model_data_phase2 %>% 
  filter(pregnant_new == 0)

# Filtering out medication users
r00_model_data_meds <- r00_model_data_phase2_nopreg %>% 
  #filtering out bp med users
  filter(is.na(med_bp)) 

# Dataset for participants who have AIx data for all 6 Phases (107 participants)
r00_model_data_6phases <- no_preg_data %>% 
  group_by(house_id) %>% 
...
##### data filtering out 22 pregnancies
model_no_preg <- lmer(aug_index ~ stove_use_3 + pregnant_previous +
                        ns(date_sphygmo, df=6) + age_baseline +
                        waist_cm + school_bi + (1 | house_id), no_preg_data)
#summary(model_no_preg)
tidy_model_no_preg <- tidy(model_no_preg, conf.int = TRUE) %>% 
    filter(grepl('stove_use_3', term)) %>% 
    mutate(model = "no pregnancies") 


##### Only participants with phase 2 data 
model_phase2_yes <- lmer(aug_index ~ stove_use_3 + 
                          ns(date_sphygmo, df=6) + age_baseline +
                          waist_cm + school_bi + (1 | house_id), 
                          r00_model_data_phase2)
#summary(model_phase2_yes)
tidy_model_phase2_yes <- tidy(model_phase2_yes, conf.int = TRUE) %>% 
    filter(grepl('stove_use_3', term)) %>% 
    mutate(model = "data for phase 2")


##### Only participants with phase 2 data and no pregnancies
model_phase2_nopreg <- lmer(aug_index ~ stove_use_3 + 
                          ns(date_sphygmo, df=6) + age_baseline +
                          waist_cm + school_bi + (1 | house_id), 
                          r00_model_data_phase2_nopreg)
#summary(model_phase2_nopreg)
tidy_model_phase2_nopreg <- tidy(model_phase2_nopreg, conf.int = TRUE) %>% 
    filter(grepl('stove_use_3', term)) %>% 
    mutate(model = "phase 2 + no preg")


##### bp meds removed (57 obs)
...
model_full <- lmer(aug_index ~ stove_use_3 + ns(date_sphygmo, df=6) + age_baseline +
                       ...
                       (1 | house_id), r00_model_data_phase2_nopreg)
#summary(model_full)
...
aix_results_phase2 <- rbind(tidy_model_stove_use_3,
                     tidy_model_no_preg,
                     tidy_model_phase2_yes,
                     tidy_model_phase2_nopreg)

aix_results_phase56 <- rbind(tidy_model_stove_use_3,
                     tidy_model_6phases,
                     tidy_model_5phases)

aix_results <- rbind(tidy_model_phase2_nopreg,
...
```{r}
plot_estimates <- aix_results_phase56 %>%
  mutate(model = factor(model, levels = c("stove use", "6 phases", "<6 phases"))) %>% 
...
plot_estimates <- aix_results_phase2 %>%
  mutate(model = factor(model, levels = c("stove use", "no pregnancies", 
                                          "data for phase 2", "phase 2 + no preg"))) %>% 
  mutate(level = factor(term, 
                        levels = c("stove_use_3justa+trad", "stove_use_3justa/imprvd"),
                        labels = c("justa+trad", "justa/imprvd"))) %>% 
  ggplot(aes(group = level, shape = level)) +
...
plot_estimates <- aix_results %>%
  mutate(model = factor(model, levels = c("phase 2 + no preg", 
                                          "no bp meds", 
...
##### data filtering out 22 pregnancies
model_no_preg <- lmer(pulse_pressure_central ~ stove_use_3 + pregnant_previous +
                        ns(date_sphygmo, df=6) + age_baseline +
                        waist_cm + school_bi + (1 | house_id), no_preg_data)
#summary(model_no_preg)
tidy_model_no_preg <- tidy(model_no_preg, conf.int = TRUE) %>% 
...
##### Only participants with phase 2 data 
model_phase2_yes <- lmer(pulse_pressure_central ~ stove_use_3 + 
                          ns(date_sphygmo, df=6) + age_baseline +
                          waist_cm + school_bi + (1 | house_id), 
                          r00_model_data_phase2)
#summary(model_phase2_yes)
...
##### Only participants with phase 2 data and no pregnancies
model_phase2_nopreg <- lmer(pulse_pressure_central ~ stove_use_3 + 
                          ns(date_sphygmo, df=6) + age_baseline +
                          waist_cm + school_bi + (1 | house_id), 
                          r00_model_data_phase2_nopreg)
#summary(model_phase2_nopreg)
...
cpp_results_phase2 <- rbind(tidy_model_stove_use_3,
                     tidy_model_no_preg,
                     tidy_model_phase2_yes,
                     tidy_model_phase2_nopreg)

cpp_results_phase56 <- rbind(tidy_model_stove_use_3,
                     tidy_model_6phases,
                     tidy_model_5phases)

cpp_results <- rbind(tidy_model_phase2_nopreg,
                     tidy_model_no_bp_meds,
                     tidy_model_phase,
                     tidy_model_season,
                     tidy_model_spline12,
...
```{r}
plot_estimates <- cpp_results_phase56 %>%
  mutate(model = factor(model, levels = c("stove use", "6 phases", "<6 phases"))) %>% 
  mutate(level = factor(term, 
                        levels = c("stove_use_3justa+trad", "stove_use_3justa/imprvd"),
                        labels = c("justa+trad", "justa/imprvd"))) %>% 
...
plot_estimates <- cpp_results_phase2 %>%
  mutate(model = factor(model, levels = c("stove use", "no pregnancies", 
                                          "data for phase 2", "phase 2 + no preg"))) %>% 
  mutate(level = factor(term, 
                        levels = c("stove_use_3justa+trad", "stove_use_3justa/imprvd"),
