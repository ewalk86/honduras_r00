...
sums_temp40_clean <- sums_all_phases_clean %>% 
  ...
  mutate(time_diff = exposure_datetime - exposure_datetime_lag) %>% 
  # first event of recording session = 1, new event (>1hr apart) = 2, end event = 3
  mutate(temp_40_new_event = if_else(time_diff>3600, 3, 0)) %>% 
  mutate(temp_40_new_event = if_else(lag(temp_40_new_event) == 3, 2, temp_40_new_event)) %>% 
...
sums_temp45_clean <- sums_all_phases_clean %>% 
  ...
  mutate(time_diff = exposure_datetime - exposure_datetime_lag) %>% 
  # first event of recording session = 1, new event (>1hr apart) = 2, end event = 3
  mutate(temp_45_new_event = if_else(time_diff>3600, 3, 0)) %>% 
  mutate(temp_45_new_event = if_else(lag(temp_45_new_event) == 3, 2, temp_45_new_event)) %>% 
...
sums_temp50_clean <- sums_all_phases_clean %>% 
  ...
  mutate(time_diff = exposure_datetime - exposure_datetime_lag) %>% 
  # first event of recording session = 1, new event (>1hr apart) = 2, end event = 3
  mutate(temp_50_new_event = if_else(time_diff>3600, 3, 0)) %>% 
  mutate(temp_50_new_event = if_else(lag(temp_50_new_event) == 3, 2, temp_50_new_event)) %>% 
...
sums_temp38_secondary_clean <- sums_all_phases_clean %>% 
  ...
  mutate(time_diff = exposure_datetime - exposure_datetime_lag) %>% 
  # first event of recording session = 1, new event (>1hr apart) = 2, end event = 3
  mutate(temp_38_new_event = if_else(time_diff>3600, 3, 0)) %>% 
  mutate(temp_38_new_event = if_else(lag(temp_38_new_event) == 3, 2, temp_38_new_event)) %>% 
  mutate(temp_38_new_event = if_else(is.na(time_diff), 1, temp_38_new_event)) %>% 
  ungroup() %>% 
  mutate(temp_38_new_event = if_else(lead(temp_38_new_event) == 1, 3, temp_38_new_event)) %>% 
  mutate(temp_38_new_event = if_else(is.na(time_diff), 1, temp_38_new_event)) %>% 
...
# Join with full dataset and save
```{r}
r00_full_repeated_by_phase <- r00_full_repeated_by_phase %>% 
  mutate(phase = as.factor(phase)) %>% 
  full_join(r00_sums_full_dataset, by = c("house_id", "phase"))
