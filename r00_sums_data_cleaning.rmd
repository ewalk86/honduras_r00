...
r00_full_repeated_by_phase <- r00_full_repeated_by_phase %>% 
  select(-prim_sum_temp_percent_38:-sec_sum_mean_event_time_50) %>% 
  mutate(phase = as.factor(phase)) %>% 
...
```{r}
r00_new <- r00_full_repeated_by_phase %>% 
  mutate(sec_stove_sums = if_else(sec_sum_event_count_50 > 0, 1, 0)) 
  

write_rds(r00_new, "output/r00_full_repeated_by_phase.rds")
write_csv(r00_new, "output/r00_full_repeated_by_phase.csv", na = " ")
```
