...
r00_model_data <- r00_full_long %>% 
  ...
  left_join(ground_weather_data, by = c("date_sphygmo")) %>%
...
model_apm_bpjoined <- lmer(sys_bp_joined ~ log_a_twa + ns(date_sphygmo, df=6) + age_baseline + 
...
# Try: temp_c, temp_rolling_24, temp_f, temp_max_previous, kitchen_temp
model_apm_temp <- lmer(outcome ~ log_a_twa + ns(date_sphygmo, df=6) + age_baseline +
                       waist_cm + school_bi + temp_f +
                       (1 | house_id), r00_model_data)
...
model_p1 <- lm(outcome ~ log_a_twa + kitchen_temp +
...
model_p2 <- lm(outcome ~ log_a_twa + kitchen_temp +
...
model_p3 <- lm(outcome ~ log_a_twa + kitchen_temp +
...
model_p4 <- lm(outcome ~ log_a_twa + kitchen_temp +
...
model_p5 <- lm(outcome ~ log_a_twa + kitchen_temp +
...
model_p6 <- lm(outcome ~ log_a_twa + kitchen_temp +
...
model_rainy <- lmer(outcome ~ log_a_twa + kitchen_temp +
                   age_baseline + waist_cm + school_bi +
                   ns(date_sphygmo, df=6) + (1 | house_id), season_rainy)
# summary(model_rainy)
...
model_dry <- lmer(outcome ~ log_a_twa + kitchen_temp +
                   age_baseline + waist_cm + school_bi +
                   ns(date_sphygmo, df=6) + (1 | house_id), season_dry)
# summary(model_dry)
...
############################################################
##### personal PM initial model - model with kitchen or ambient temp
# Try: temp_c, temp_rolling_24, temp_f, temp_max_previous, kitchen_temp
...
model_p1 <- lm(outcome ~ log_p_twa + kitchen_temp +
                   age_baseline + waist_cm + school_bi , phase_1)
# summary(model_p1)
tidy_model_p1 <- tidy(model_p1, conf.int = TRUE) %>% 
    filter(grepl('log_p_twa', term)) %>%  
...
model_p2 <- lm(outcome ~ log_p_twa + kitchen_temp +
                   age_baseline + waist_cm + school_bi , phase_2)
# summary(model_p2)
tidy_model_p2 <- tidy(model_p2, conf.int = TRUE) %>% 
    filter(grepl('log_p_twa', term)) %>%  
...
model_p3 <- lm(outcome ~ log_p_twa + kitchen_temp +
                   age_baseline + waist_cm + school_bi , phase_3)
# summary(model_p3)
tidy_model_p3 <- tidy(model_p3, conf.int = TRUE) %>% 
    filter(grepl('log_p_twa', term)) %>%  
...
model_p4 <- lm(outcome ~ log_p_twa + kitchen_temp +
                   age_baseline + waist_cm + school_bi , phase_4)
# summary(model_p4)
tidy_model_p4 <- tidy(model_p4, conf.int = TRUE) %>% 
    filter(grepl('log_p_twa', term)) %>%  
...
model_p5 <- lm(outcome ~ log_p_twa + kitchen_temp +
                   age_baseline + waist_cm + school_bi , phase_5)
# summary(model_p5)
tidy_model_p5 <- tidy(model_p5, conf.int = TRUE) %>% 
    filter(grepl('log_p_twa', term)) %>%  
...
model_p6 <- lm(outcome ~ log_p_twa + kitchen_temp +
                   age_baseline + waist_cm + school_bi , phase_6)
# summary(model_p6)
tidy_model_p6 <- tidy(model_p6, conf.int = TRUE) %>% 
    filter(grepl('log_p_twa', term)) %>%  
...
model_rainy <- lmer(outcome ~ log_p_twa + kitchen_temp +
                   age_baseline + waist_cm + school_bi +
                   ns(date_sphygmo, df=6) + (1 | house_id), season_rainy)
# summary(model_rainy)
tidy_model_rainy <- tidy(model_rainy, conf.int = TRUE) %>% 
...
model_dry <- lmer(outcome ~ log_p_twa + kitchen_temp +
                   age_baseline + waist_cm + school_bi +
                   ns(date_sphygmo, df=6) + (1 | house_id), season_dry)
# summary(model_dry)
tidy_model_dry <- tidy(model_dry, conf.int = TRUE) %>% 
