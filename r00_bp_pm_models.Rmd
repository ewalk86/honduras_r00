...
model_obs_kpm_sbp <- as.data.frame(nobs(kpm_sbp)) %>% 
  rename(N = `nobs(kpm_sbp)`) %>% 
  mutate(model = "Systolic BP",
         Exposure = "Kitchen PM")

kpm_dbp <- read_rds("output/kpm_dbp.rds")
model_obs_kpm_dbp <- as.data.frame(nobs(kpm_dbp)) %>% 
  rename(N = `nobs(kpm_dbp)`) %>% 
...
model_obs_ppm_sbp <- as.data.frame(nobs(ppm_sbp)) %>% 
  rename(N = `nobs(ppm_sbp)`) %>% 
  mutate(model = "Systolic BP",
         Exposure = "Personal PM")

ppm_dbp <- read_rds("output/ppm_dbp.rds")
model_obs_ppm_dbp <- as.data.frame(nobs(ppm_dbp)) %>% 
  rename(N = `nobs(ppm_dbp)`) %>% 
  mutate(model = "Diastolic BP",
         Exposure = "Personal PM")
...
model_obs_kbc_sbp <- as.data.frame(nobs(kbc_sbp)) %>% 
  rename(N = `nobs(kbc_sbp)`) %>% 
  mutate(model = "Systolic BP",
         Exposure = "Kitchen BC")

kbc_dbp <- read_rds("output/kbc_dbp.rds")
model_obs_kbc_dbp <- as.data.frame(nobs(kbc_dbp)) %>% 
  rename(N = `nobs(kbc_dbp)`) %>% 
  mutate(model = "Diastolic BP",
         Exposure = "Kitchen BC")
...
model_obs_pbc_sbp <- as.data.frame(nobs(pbc_sbp)) %>% 
  rename(N = `nobs(pbc_sbp)`) %>% 
  mutate(model = "Systolic BP",
         Exposure = "Personal BC")

pbc_dbp <- read_rds("output/pbc_dbp.rds")
model_obs_pbc_dbp <- as.data.frame(nobs(pbc_dbp)) %>% 
  rename(N = `nobs(pbc_dbp)`) %>% 
  mutate(model = "Diastolic BP",
         Exposure = "Personal BC")
...
model_obs_kpm_lta_sbp <- as.data.frame(nobs(kpm_lta_sbp)) %>% 
  rename(N = `nobs(kpm_lta_sbp)`) %>% 
  mutate(model = "Systolic BP",
         Exposure = "Kitchen PM, LTA")

kpm_lta_dbp <- read_rds("output/kpm_lta_dbp.rds")
model_obs_kpm_lta_dbp <- as.data.frame(nobs(kpm_lta_dbp)) %>% 
  rename(N = `nobs(kpm_lta_dbp)`) %>% 
  mutate(model = "Diastolic BP",
         Exposure = "Kitchen PM, LTA")
...
model_obs_ppm_lta_sbp <- as.data.frame(nobs(ppm_lta_sbp)) %>% 
  rename(N = `nobs(ppm_lta_sbp)`) %>% 
  mutate(model = "Systolic BP",
         Exposure = "Personal PM, LTA")

ppm_lta_dbp <- read_rds("output/ppm_lta_dbp.rds")
model_obs_ppm_lta_dbp <- as.data.frame(nobs(ppm_lta_dbp)) %>% 
  rename(N = `nobs(ppm_lta_dbp)`) %>% 
  mutate(model = "Diastolic BP",
         Exposure = "Personal PM, LTA")
...
model_obs_kbc_lta_sbp <- as.data.frame(nobs(kbc_lta_sbp)) %>% 
  rename(N = `nobs(kbc_lta_sbp)`) %>% 
  mutate(model = "Systolic BP",
         Exposure = "Kitchen BC, LTA")

kbc_lta_dbp <- read_rds("output/kbc_lta_dbp.rds")
model_obs_kbc_lta_dbp <- as.data.frame(nobs(kbc_lta_dbp)) %>% 
  rename(N = `nobs(kbc_lta_dbp)`) %>% 
  mutate(model = "Diastolic BP",
         Exposure = "Kitchen BC, LTA")
...
model_obs_pbc_lta_sbp <- as.data.frame(nobs(pbc_lta_sbp)) %>% 
  rename(N = `nobs(pbc_lta_sbp)`) %>% 
  mutate(model = "Systolic BP",
         Exposure = "Personal BC, LTA")

pbc_lta_dbp <- read_rds("output/pbc_lta_dbp.rds")
model_obs_pbc_lta_dbp <- as.data.frame(nobs(pbc_lta_dbp)) %>% 
  rename(N = `nobs(pbc_lta_dbp)`) %>% 
  mutate(model = "Diastolic BP",
         Exposure = "Personal BC, LTA")
...
pm_obs_combined <- rbind(model_obs_kpm_sbp, model_obs_kpm_dbp,
                         model_obs_ppm_sbp, model_obs_ppm_dbp,
                         model_obs_kpm_lta_sbp, model_obs_kpm_lta_dbp,
                         model_obs_ppm_lta_sbp, model_obs_ppm_lta_dbp) %>% 
  rename(Model = model) %>% 
  select(Model, Exposure, N) %>% 
  mutate(Model = factor(Model, levels = c("Systolic BP", "Diastolic BP")),
         Exposure = factor(Exposure, levels = c("Kitchen PM", "Kitchen PM, LTA",
                                      "Personal PM", "Personal PM, LTA"))) %>% 
  arrange(Model, Exposure)

bc_obs_combined <- rbind(model_obs_kbc_sbp, model_obs_kbc_dbp,
                         model_obs_pbc_sbp, model_obs_pbc_dbp,
                         model_obs_kbc_lta_sbp, model_obs_kbc_lta_dbp,
                         model_obs_pbc_lta_sbp, model_obs_pbc_lta_dbp) %>% 
  rename(Model = model) %>% 
  select(Model, Exposure, N) %>% 
  mutate(Model = factor(Model, levels = c("Systolic BP", "Diastolic BP")),
         Exposure = factor(Exposure, levels = c("Kitchen BC", "Kitchen BC, LTA",
                                      "Personal BC", "Personal BC, LTA"))) %>% 
  arrange(Model, Exposure)
...
all_pm_combined <- rbind(kpm_sbp_tidy, kpm_dbp_tidy,
                     ppm_sbp_tidy, ppm_dbp_tidy,
                     kpm_lta_sbp_tidy, kpm_lta_dbp_tidy, 
                     ppm_lta_sbp_tidy, ppm_lta_dbp_tidy)

all_bc_combined <- rbind(kbc_sbp_tidy, kbc_dbp_tidy, 
                     pbc_sbp_tidy, pbc_dbp_tidy, 
...
pm_plot_estimates <- all_pm_combined %>%
  ...
  labs(x = "", group = "", shape = "") +
  scale_y_continuous(breaks = c(seq(-1, 2, 0.5)), labels = c(seq(-1, 2, 0.5))) +
  theme(title = element_text(size = 16), 
          axis.text.x = element_text(size = 26, colour = "black", angle = 25,
                                     hjust = 0.5, vjust = 0.5),
  ...
  scale_shape_manual(values = c(15, 16, 17, 18),
                     name = "Model") +
  annotate(geom = "table", x = 4.5, y = 1.5, label = list(pm_obs_combined),
           color = "black", fill = "white", size = 9)
pm_plot_estimates

# Save plot as jpg
ggsave("pm_plot_estimates.jpg", width = 14, height = 7)
...
bc_plot_estimates <- all_bc_combined %>%
  ...
  labs(x = "", group = "", shape = "") +
  scale_y_continuous(breaks = c(seq(-0.4, 1.0, 0.2)), labels = c(seq(-0.4, 1.0, 0.2))) +
  theme(title = element_text(size = 16), 
          axis.text.x = element_text(size = 26, colour = "black", angle = 25,
                                     hjust = 0.5, vjust = 0.5),
          axis.text.y = element_text(size = 26, colour = "black"),
  ...
  scale_shape_manual(values = c(15, 16, 17, 18),
                     name = "Model") +
  annotate(geom = "table", x = 4.5, y = 0.8, label = list(bc_obs_combined),
           color = "black", fill = "white", size = 9)
bc_plot_estimates

# Save plot as jpg
ggsave("bc_plot_estimates.jpg", width = 14, height = 7)
```
