...
r00_model_data_outliers <- r00_model_data %>% 
...
sphygmo_model_season_outliers <- lmer(aug_index ~ assigned_stove + season + 
                          (1 | house_id), r00_model_data_outliers)
...
## The same 6 observations were also removed for Pulse Pressure 
```{r}
...
tidy_sphygmo_model_season_outliers <- tidy(sphygmo_model_season_outliers, conf.int = TRUE) %>% 
    filter(grepl('assigned', term)) %>% 
    rename(ci_low = conf.low, ci_hi = conf.high) %>% 
    mutate(model = if_else(term == "assigned_stoveJusta", "6 AIx outliers removed", "na")) 
...
plot(sphygmo_model_season_outliers, 
     main = "Central Pulse Pressure - 6 AIx outliers removed", 
     xlab = "fitted values", ylab = "residuals")

qqnorm(residuals(sphygmo_model_season_outliers), 
       main = "Central Pulse Pressure - 6 AIx outliers removed QQ Plot")
