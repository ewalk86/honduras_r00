...
model_confounders <- lmer(outcome ~ mean_log_a_twa + ns(date_sphygmo, df=6) +  
...
tidy_model_confounders <- tidy(model_confounders, conf.int = TRUE) %>% 
    filter(grepl('mean_log_a_twa', term)) %>% 
    mutate(model = if_else(term == "mean_log_a_twa", "confounders", "na"))
