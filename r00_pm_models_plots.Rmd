...
model_apm_full <- lmer(aug_index ~ log_a_twa + ns(date_sphygmo, df=6) + age_baseline +
                      whr + school_self + dds_total + phys_act +
...
plot_estimates <- aix_results %>%
  mutate(model = factor(model, levels = c("primary", "6 phases", "reduced",
                                          "spline 12 df", "phase", "season",
                                          "bmi", "beds", "ses assets", "dds",
                                          "phys act", "full"))) %>% 
  ggplot() +
...
model_apm_full <- lmer(pulse_pressure_central ~ log_a_twa + ns(date_sphygmo, df=6) + age_baseline +
                      whr + school_self + dds_total + phys_act +
                        ses_materials + beds_new + 
                        (1 | house_id), r00_model_data)
#summary(model_apm_full)
