...
```{r, echo=TRUE}
##### AIx and area PM 
model_apm_phase2 <- lmer(aug_index ~ log_a_twa + ns(date_sphygmo, df=6) + age_baseline +
                      waist_cm + school_bi + (1 | house_id), r00_model_data_phase2)
summary(model_apm_phase2)
tidy_model_apm_phase2 <- tidy(model_apm_phase2, conf.int = TRUE) %>% 
...
model_apm_bmi <- lmer(aug_index ~ log_a_twa + ns(date_sphygmo, df=6) + age_baseline +
                      bmi + school_bi + (1 | house_id), r00_model_data_phase2)
...
model_apm_whr <- lmer(aug_index ~ log_a_twa + ns(date_sphygmo, df=6) + age_baseline +
                      whr + school_bi + (1 | house_id), r00_model_data_phase2)
...
aix_results <- rbind(tidy_model_apm_phase2,
                     tidy_model_apm,
...
plot_estimates <- aix_results %>%
  mutate(model = factor(model, levels = c("primary", "all data", "no bp meds", "6 phases",
...
# AIx and personal PM
```{r, echo=TRUE}
##### AIx and personal PM primary 
model_ppm_phase2 <- lmer(aug_index ~ log_p_twa + ns(date_sphygmo, df=6) + age_baseline +
                  waist_cm + school_bi + (1 | house_id), r00_model_data_phase2)
summary(model_ppm_phase2)
tidy_model_ppm_phase2 <- tidy(model_ppm_phase2, conf.int = TRUE) %>% 
...
model_ppm_di_conf <- lmer(aug_index ~ log_p_twa + ns(date_sphygmo, df=6) + age_cat_40 +
                          waist_cat + school_bi + (1 | house_id), r00_model_data_phase2)
...
aix_results <- rbind(tidy_model_ppm_phase2,
                     tidy_model_ppm,
...
model_apm_phase2 <- lmer(pulse_pressure_central ~ log_a_twa + 
                  ns(date_sphygmo, df=6) + age_baseline +
                  waist_cm + school_bi + (1 | house_id), r00_model_data_phase2)
summary(model_apm_phase2)
tidy_model_apm_phase2 <- tidy(model_apm_phase2, conf.int = TRUE) %>% 
...
cpp_results <- rbind(tidy_model_apm_phase2,
                     tidy_model_apm,
                     tidy_model_apm_no_bp_meds,
                     tidy_model_apm_6phases,
                     tidy_model_apm_phase,
...
plot_estimates <- cpp_results %>%
  ...
  theme_bw() +  
  #ggtitle(label = "AIx sensitivity analyses: area PM model") +
  labs(y = "Estimate per natural log increase in \n fine particulate matter (mmHg)") +
  labs(x = "") +
  theme(title = element_text(size = 16), 
          axis.text.x = element_text(size = 16, colour = "black", angle = 25,
                                     hjust = 1, vjust = .8),
...
model_ppm_phase2 <- lmer(pulse_pressure_central ~ log_p_twa + 
                  ns(date_sphygmo, df=6) + age_baseline +
                  waist_cm + school_bi + (1 | house_id), r00_model_data_phase2)
summary(model_ppm_phase2)
tidy_model_ppm_phase2 <- tidy(model_ppm_phase2, conf.int = TRUE) %>% 
...
cpp_results <- rbind(tidy_model_ppm_phase2,
                     tidy_model_ppm,
                     tidy_model_ppm_no_bp_meds,
                     tidy_model_ppm_6phases,
                     tidy_model_ppm_phase,
