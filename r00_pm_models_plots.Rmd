...
plot_estimates <- aix_results %>%
  mutate(model = factor(model, levels = c("primary", "no bp meds", "6 phases",
                                          ...
                                          "phys act"))) %>% 
  ggplot() +
  geom_point(aes(x=model, y=estimate, color = model, shape = model), size = 4) +
  scale_shape_manual(values = c(15, 16, 17, 18, 13, 9, 4, 8, 14, 10, 12, 7, 11)) +
  geom_errorbar(aes(x=model, ymin=conf.low, ymax=conf.high, color = model), 
                size = 1.2, width = 0.5) +
...
plot_estimates <- aix_results %>%
  ...
  geom_hline(yintercept = 0) +
  theme_bw() +  
  ggtitle(label = "AIx sensitivity analyses: personal PM model") +
  labs(y = "Estimate (%)") +
  labs(x = "") +
  theme(title = element_text(size = 16), 
          axis.text.x = element_blank(),
...
plot_estimates <- cpp_results %>%
  ...
  geom_hline(yintercept = 0) +
  theme_bw() +  
  ggtitle(label = "CPP sensitivity analyses: area PM model") +
...
plot_estimates <- cpp_results %>%
  ...
  geom_hline(yintercept = 0) +
  theme_bw() +  
  ggtitle(label = "CPP sensitivity analyses: personal PM model") +
  labs(y = "Estimate (mmHg)") +
  labs(x = "") +
  theme(title = element_text(size = 16), 
          axis.text.x = element_blank(),
