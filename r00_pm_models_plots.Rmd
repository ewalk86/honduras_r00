...
model_apm_temp <- lmer(aug_index ~ log_a_twa + ns(date_sphygmo, df=6) + age_baseline +
...
# Try: temp_c, temp_rolling_24, temp_max_previous, temp_max, kitchen_temp,
# ambient_pm, ambient_bc, ambient_oc
model_ppm_temp <- lmer(aug_index ~ log_p_twa + ns(date_sphygmo, df=6) + age_baseline +
                       waist_cm + school_bi + temp_c +
                       (1 | house_id), r00_model_data)
...
# Try: temp_c, temp_rolling_24, temp_max_previous, temp_max, kitchen_temp,
# ambient_pm, ambient_bc, ambient_oc
model_apm_temp <- lmer(pulse_pressure_central ~ log_a_twa 
                       + ns(date_sphygmo, df=6) + age_baseline +
                       waist_cm + school_bi + temp_max +
                       (1 | house_id), r00_model_data)
#summary(model_apm_temp)
tidy_model_apm_temp <- tidy(model_apm_temp, conf.int = TRUE) %>% 
    filter(grepl('log_a_twa', term)) %>% 
...
# Try: temp_c, temp_rolling_24, temp_max_previous, temp_max, kitchen_temp,
# ambient_pm, ambient_bc, ambient_oc
model_ppm_temp <- lmer(pulse_pressure_central ~ log_p_twa 
                       + ns(date_sphygmo, df=6) + age_baseline +
                       waist_cm + school_bi + log(ambient_bc) +
                       (1 | house_id), r00_model_data)
#summary(model_ppm_temp)
tidy_model_ppm_temp <- tidy(model_ppm_temp, conf.int = TRUE) %>% 
    filter(grepl('log_p_twa', term)) %>% 
