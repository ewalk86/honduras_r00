...
tidy_model_apm_5phases <- tidy(model_apm_5phases, conf.int = TRUE) %>% 
    filter(grepl('log_a_twa', term)) %>% 
    mutate(model = if_else(term == "log_a_twa", "missed phase", "na")) 


##### AIx and area PM - participants who missed phase 2 - sphygmo malfunction filtered out
...
tidy_model_p2_sphygmo <- tidy(model_p2_sphygmo, conf.int = TRUE) %>% 
    filter(grepl('log_a_twa', term)) %>% 
    mutate(model = if_else(term == "log_a_twa", "missed phase 2", "na")) 
...
tidy_model_ppm_5phases <- tidy(model_ppm_5phases, conf.int = TRUE) %>% 
    filter(grepl('log_p_twa', term)) %>% 
    mutate(model = if_else(term == "log_p_twa", "missed phase", "na"))
...
tidy_model_p2_sphygmo <- tidy(model_p2_sphygmo, conf.int = TRUE) %>% 
    filter(grepl('log_p_twa', term)) %>% 
    mutate(model = if_else(term == "log_p_twa", "missed phase 2", "na")) 
