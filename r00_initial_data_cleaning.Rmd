...
inflammatory_data <- read_xlsx("input/r00_dbs_data_20191004.xlsx")

inflammatory_data_clean <- inflammatory_data %>% 
  separate(dbs_A_phase, c("p", "phase"), sep = "p") %>% 
  mutate(il1b_dbs_a = as.numeric(DBS_A_IL1b),
         il6_dbs_a = as.numeric(DBS_A_IL6),
         il8_dbs_a = as.numeric(DBS_A_IL8),
         tnfa_dbs_a = as.numeric(DBS_A_TNFa),
         tnfa_2_dbs_a = as.numeric(DBS_A_TNFa_2),
         crp_dbs_a = as.numeric(DBS_A_CRP)) %>% 
  select(-p, -DBS_A_IL1b, -DBS_A_IL6, -DBS_A_IL8, -DBS_A_TNFa, -DBS_A_TNFa_2, -DBS_A_CRP)

full_inflammatory_joined <- r00_full_long %>% 
  left_join(inflammatory_data_clean, by = c("house_id", "phase")) %>% 
  select(-il1b, -il6, -il8, -tnfa, -tnfa_2, -crp)
```

# Save final, combined dataset
```{r}
write_rds(full_inflammatory_joined, "output/r00_full_repeated_by_phase.rds")

write_csv(full_inflammatory_joined, "output/r00_full_repeated_by_phase.csv", na = " ")
```

# Remove AIx/CPP outliers and save dataset
```{r}
# load full dataset
r00_full_long <- read_rds("output/r00_full_repeated_by_phase.rds")
