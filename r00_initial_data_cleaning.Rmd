...
```{r, message=FALSE}
# Load data
r00_full_wide <- read_xlsx("input/R00_health_wide_01July2019_by.xlsx")
...
test <- r00_joined_3 %>% 
  select(house_id, phase, co_a_15minmax.x, co_a_15minmax.y, mean_temp.x, mean_temp.y, 
         a_pm.x, a_pm.y, log_a_pm25.x, log_a_pm25.y)
```
...
r00_final_long <- r00_joined_3 %>% 
  ...
  select(-pregnant2, -pregnant3, -pregnant_test, -child_born_date, -date_diff) %>% 
  ungroup()
```
