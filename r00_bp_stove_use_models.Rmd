...
model_obs_sbp <- as.data.frame(table(model.frame(su_sbp)$stove_use_3)) %>% 
  rename(term = Var1,
         N = Freq) %>% 
  mutate(model = "Systolic BP")

su_dbp <- read_rds("output/su_dbp.rds")
model_obs_dbp <- as.data.frame(table(model.frame(su_dbp)$stove_use_3)) %>% 
  rename(term = Var1,
         N = Freq) %>% 
  mutate(model = "Diastolic BP")

su_aix <- read_rds("output/su_aix.rds")
model_obs_aix <- as.data.frame(table(model.frame(su_aix)$stove_use_3)) %>% 
  rename(term = Var1,
         N = Freq) %>% 
  mutate(model = "Aug Index")

su_cpp <- read_rds("output/su_cpp.rds")
model_obs_cpp <- as.data.frame(table(model.frame(su_cpp)$stove_use_3)) %>% 
  rename(term = Var1,
         N = Freq) %>% 
  mutate(model = "Cent Pulse Press")

su_obs_combined <- rbind(model_obs_sbp, model_obs_dbp) %>% 
  rename(Model = model,
         Stove = term) %>% 
  mutate(Stove = factor(Stove,
                       levels = c("trad", "justa/imprvd", "justa+trad"),
                       labels = c("Traditional", "Justa/Improved", "Justa + Trad"))) %>% 
...
tidy_results <- tidy(model_results, conf.int = TRUE) %>% 
  ...
  mutate(term = factor(term,
                       ...
                       labels = c("Justa/Improved", "Justa + Trad")),
         model = model_name) %>% 
...
su_combined <- rbind(su_sbp_tidy, su_dbp_tidy)
...
su_plot_estimates <- su_combined %>%
  ...
  labs(x = "", group = "", shape = "") +
  scale_y_continuous(breaks = c(seq(-2, 2, 0.5)), labels = c(seq(-2, 2, 0.5))) +
  theme(title = element_text(size = 16), 
  ...
  scale_shape_manual(values = c(15, 16, 17, 18),
                     name = "Model") +
  annotate(geom = "table", x = 4.5, y = 1.25, label = list(su_obs_combined),
           color = "black", fill = "white", size = 9)
su_plot_estimates

# Save plot as jpg
ggsave("su_plot_estimates.jpg", width = 14, height = 7)
```
