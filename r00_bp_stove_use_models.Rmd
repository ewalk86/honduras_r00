...
obs_combined <- rbind(model_obs_sbp, model_obs_dbp, model_obs_aix, model_obs_cpp) %>% 
  rename(Model = model,
         Stove = term) %>% 
...
su_plot_estimates <- su_combined %>%
  ...
  theme_bw() +  
  #geom_hline(yintercept = 0) +
  annotate("segment", x = 0, xend = 4.5, y = 0, yend = 0, size = 1.2) +
  ggtitle(label = "") +
  ...
  scale_shape_manual(values = c(15, 16, 17, 18),
                     name = "Model") +
  annotate(geom = "table", x = 7, y = 4, label = list(obs_combined),
