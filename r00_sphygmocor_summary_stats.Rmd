...
summary_aix_stove_use <- r00_model_data_outliers_removed %>% 
  filter(!is.na(assigned_stove)) %>% 
  filter(!is.na(aug_index)) %>% 
  select(stove_use, assigned_stove, aug_index) %>% 
  group_by(stove_use) %>% 
  summarise("Mean" = mean(aug_index),
            ...
            "Max" = max(aug_index),
            "IQR" = IQR(aug_index))
kable(summary_aix_stove_use, digits = 0)
```
...
summary_cpp_stove_use <- r00_model_data_outliers_removed %>% 
  ...
  select(stove_use, assigned_stove, pulse_pressure_central) %>% 
  group_by(stove_use) %>% 
...
summary_stats_phase6 <- r00_model_data_6phases %>% 
  filter(!is.na(assigned_stove)) %>% 
  filter(!is.na(aug_index)) %>%
  select(phase, "Augmentation Index" = aug_index, "Heart Rate" = hr, 
         "Systolic BP Peripheral" = sys_bp_periph, "Diastolic BP Peripheral" = dia_bp_periph, 
...
summary_stats_phase5 <- r00_model_data_5phases %>% 
  filter(!is.na(assigned_stove)) %>% 
  filter(!is.na(aug_index)) %>%
  select(phase, "Augmentation Index" = aug_index, "Heart Rate" = hr, 
         "Systolic BP Peripheral" = sys_bp_periph, "Diastolic BP Peripheral" = dia_bp_periph, 
