...
# AIx grouped by stove use - all phases
```{r}
...
summary_aix_stove_use_phase2_no <- r00_data_phase2_no %>% 
  filter(!is.na(assigned_stove)) %>% 
  filter(!is.na(aug_index)) %>% 
  select(stove_use_3, assigned_stove, aug_index, phase, study_arm) %>% 
  group_by(stove_use_3) %>% 
  ...
  summarise("Mean" = mean(aug_index, na.rm = TRUE),
            ...
            "Max" = max(aug_index),
            "IQR" = IQR(aug_index))
kable(summary_aix_stove_use_phase2_no, digits = 1)

summary_aix_stove_use_phase2_yes <- r00_data_phase2_yes %>% 
  filter(!is.na(assigned_stove)) %>% 
  filter(!is.na(aug_index)) %>% 
  select(stove_use_3, assigned_stove, aug_index, phase, study_arm) %>% 
  group_by(stove_use_3) %>% 
  ...
  summarise("Mean" = mean(aug_index, na.rm = TRUE),
            ...
            "Max" = max(aug_index),
            "IQR" = IQR(aug_index))
kable(summary_aix_stove_use_phase2_yes, digits = 1)
```
...
summary_cpp_stove_use_phase2_no <- r00_data_phase2_no %>% 
  filter(!is.na(assigned_stove)) %>% 
  filter(!is.na(pulse_pressure_central)) %>% 
  select(stove_use_3, assigned_stove, pulse_pressure_central, phase, study_arm) %>% 
  group_by(study_arm) %>% 
  #group_by(stove_use_3, phase, study_arm) %>% 
  summarise("Mean" = mean(pulse_pressure_central, na.rm = TRUE),
            "SD" = sd(pulse_pressure_central), 
            "n" = n(),
            "%" = n()/301*100,
            "25th" = quantile(pulse_pressure_central, c(.25)),
...
summary_cpp_stove_use_phase2_yes <- r00_data_phase2_yes %>% 
  filter(!is.na(assigned_stove)) %>% 
  filter(!is.na(pulse_pressure_central)) %>% 
  select(stove_use_3, assigned_stove, pulse_pressure_central, phase, study_arm) %>% 
  group_by(study_arm) %>% 
