...
aix_anova_data <- r00_model_data_outliers_removed %>% 
  select(study_arm, aug_index, house_id, pulse_pressure_central, assigned_stove) %>% 
...
### Data from phase 1
```{r}
summary_stats <- data_outliers_removed %>% 
  filter(!is.na(aug_index)) %>% 
...
### Data from phase 1
```{r}
summary_stats_all <- data_outliers_removed %>% 
...
summary_aix_stove_use <- data_outliers_removed %>% 
  #filter(!is.na(assigned_stove)) %>% 
  #filter(!is.na(aug_index)) %>% 
...
summary_aix_study_arm <- data_outliers_removed %>% 
  filter(!is.na(assigned_stove)) %>% 
  filter(!is.na(aug_index)) %>% 
  select(stove_use_3, assigned_stove, aug_index, phase, study_arm,
         a_twa, p_twa, age_cat_40, waist_cat, ses_cat_2, salt) %>% 
...
summary_aix <- data_outliers_removed %>% 
  filter(!is.na(assigned_stove)) %>% 
  filter(!is.na(aug_index)) %>% 
  select(stove_use_3, assigned_stove, aug_index, phase, study_arm,
         a_twa, p_twa, age_cat_40, waist_cat, ses_cat_2, salt) %>% 
...
summary_cpp_stove_use <- data_outliers_removed %>% 
  filter(!is.na(assigned_stove)) %>% 
  filter(!is.na(pulse_pressure_central)) %>% 
...
summary_cpp <- data_outliers_removed %>% 
  filter(!is.na(assigned_stove)) %>% 
  filter(!is.na(pulse_pressure_central)) %>% 
  select(stove_use_3, assigned_stove, pulse_pressure_central, phase, study_arm) %>% 
  #group_by(stove_use_3, phase, study_arm) %>% 
...
```{r}
summary_stats_study_arm <- data_outliers_removed %>%
  select(phase, study_arm, "Augmentation Index" = aug_index, "Heart Rate" = hr, 
         "Systolic BP Peripheral" = sys_bp_periph, "Diastolic BP Peripheral" = dia_bp_periph, 
         "MAP Peripheral" = map_periph, "Systolic BP Central" = sys_bp_central, 
         "Diastolic BP Central" = dia_bp_central, "MAP Central" = map_central, 
...
```{r}
summary_stats_age_cat <- data_outliers_removed %>% 
...
### Data from phase 1 and 2
```{r}
summary_stats_bmi_cat <- data_outliers_removed %>% 
...
```{r}
line_plot_data <- data_outliers_removed %>% 
...
```{r}
summary_stats_phase <- data_outliers_removed %>% 
  filter(!is.na(assigned_stove)) %>% 
  filter(!is.na(aug_index)) %>%
  select(phase, study_arm, "Augmentation Index" = aug_index, "Heart Rate" = hr, 
         "Systolic BP Peripheral" = sys_bp_periph, "Diastolic BP Peripheral" = dia_bp_periph, 
...
```{r}
summary_correlation <- data_outliers_removed %>% 
...
sphygmocor_means <- data_outliers_removed %>% 
...
sphygmocor_boxplot <- data_outliers_removed %>% 
...
phases_completed_sphygmocor <- data_outliers_removed %>%
...
```{r}
all_6_phases <- data_outliers_removed %>% 
  filter(!is.na(assigned_stove)) %>% 
...
```{r}
data_aix_check <- data_outliers_removed %>% 
...
```{r}
data_sbp_check <- data_outliers_removed %>% 
...
```{r}
data_dbp_check <- data_outliers_removed %>% 
  select(house_id, phase, study_arm, sysbp, diabp, sys_bp_periph, dia_bp_periph) %>% 
...
```{r}
# AIx
aix_histogram <- data_outliers_removed %>%
  select(phase, study_arm, aug_index) %>% 
  mutate(study_arm = factor(study_arm, levels = c(1, 2), labels = c("Arm 1", "Arm 2"))) %>%
  na.exclude(aug_index) %>% 
    ggplot()+
...
```{r}
# AIx75
aix75_histogram <- data_outliers_removed %>%
...
pulse_pressure_central_histogram <- data_outliers_removed %>%
...
```{r}
# AIx
line_plots <- data_outliers_removed %>% 
