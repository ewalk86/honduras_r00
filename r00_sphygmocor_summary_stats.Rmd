...
r00_data_phase34 <- r00_model_data_outliers_removed %>%
  #create new var to be able to filter for participants that completed 6 phases
  mutate(phase_complete = if_else(!is.na(aug_index), 1, 0)) %>% 
  group_by(house_id) %>% 
  filter(sum(phase_complete) < 6) %>%
  mutate(phase_34 = if_else(stove_use_3 == "justa/imprvd" & (phase==3 | phase==4),
                            1, 0),
         phase_34 = if_else(phase==5, lag(phase_34), phase_34),
         phase_34 = if_else(phase==6, lag(phase_34), phase_34),
         phase_34 = if_else(phase==2, lead(phase_34), phase_34),
         phase_34 = if_else(phase==1, lead(phase_34), phase_34)) %>% 
  select(house_id, phase, phase_34, study_arm, aug_index, stove_use_3, assigned_stove) %>% 
  filter(phase_34 == 1)  %>% 
  #select(house_id, phase, phase_34)
  group_by(phase) %>% 
  summarize(mean(aug_index), n())

r00_data_phase23 <- r00_model_data_outliers_removed %>%
  group_by(house_id) %>% 
  #create new var to be able to filter for participants that completed 6 phases
  mutate(phase_complete = if_else(!is.na(aug_index), 1, 0)) %>% 
  filter(sum(phase_complete) < 6) %>%
  mutate(phase_23 = if_else(stove_use_3 == "trad" & (phase==3 | phase==2),
                            1, 0),
         phase_23 = if_else(phase==4, lag(phase_23), phase_23),
         phase_23 = if_else(phase==5, lag(phase_23), phase_23),
         phase_23 = if_else(phase==6, lag(phase_23), phase_23),
         phase_23 = if_else(phase==1, lead(phase_23), phase_23)) %>% 
  select(house_id, phase, phase_23, study_arm, aug_index, stove_use_3, assigned_stove) %>% 
  filter(phase_23 == 1)  %>% 
  #select(house_id, phase, phase_23)
  #group_by(study_arm) %>% 
  summarize(mean(aug_index), n())
  
r00_highlow_data <- r00_data_phase23 %>% 
  full_join(r00_data_phase34, by = c("house_id", "phase")) %>% 
  mutate(filter_highlow = as.numeric(1))
...
test <- read_rds("output/r00_model_data_outliers_removed.RDS") %>% 
  # removes 6 observations for participant who wasn't assigned to a study arm
  filter(!is.na(assigned_stove)) %>% 
  # removes 22 observations for pregnancies
  filter(!is.na(aug_index) & phase == 2) %>% 
  filter(pregnant_new == 0) %>% 
  select(house_id, phase, aug_index, assigned_stove, pulse_pressure_central)
  # further removes 196 observations (n=1162)
  filter(!is.na(aug_index))
  
  
# 6 participants pregnant during phase 2, but they didn't have phase 2 data anyway
# 162 participants with phase 2 data, not pregnant
## Of those:
  ## 7 additional are missing phase 1 AIx data (0 pregnant)
  ## 14 additional missing phase 3 AIx data (1 additional pregnant)
  ## 14 additional missing phase 4 AIx data (1 additional pregnant)
  ## 25 additional missing phase 5 AIx data (0 additional pregnant)
  ## 30 additional missing phase 6 AIx data (0 additional pregnant)
```
...
summary_aix_stove_use <- r00_model_data_outliers_removed %>% 
  filter(!is.na(assigned_stove)) %>% 
  filter(!is.na(aug_index)) %>% 
