...
r00_data_5phases <- r00_model_data_outliers_removed %>% 
  ...
  filter(sum(phase_complete) < 6) %>%
  ungroup() %>% 
  select(house_id, phase, aug_index, stove_use_3, stove_use_4, assigned_stove, date_sphygmo)
```
...
summary_stats_stove <- r00_data_5phases %>% 
  filter(!is.na(assigned_stove)) %>% 
  filter(!is.na(aug_index)) %>% 
  select(assigned_stove, aug_index, pulse_pressure_central) %>% 
  group_by(assigned_stove) %>% 
...
summary_stats_stove <- r00_data_6phases %>% 
  filter(!is.na(assigned_stove)) %>% 
  filter(!is.na(aug_index)) %>% 
  select(assigned_stove, aug_index, pulse_pressure_central) %>% 
  group_by(assigned_stove) %>% 
...
### Data from phase 1
```{r}
summary_stats_study_arm <- r00_data_6phases %>% 
  filter(!is.na(aug_index)) %>% 
...
### Data from phase 1
```{r}
summary_stats_all <- r00_data_6phases %>% 
  filter(!is.na(aug_index)) %>% 
  filter(!is.na(assigned_stove)) %>%
  select(phase, study_arm, "Augmentation Index" = aug_index, "Heart Rate" = hr, 
         "Systolic BP Peripheral" = sys_bp_periph, "Diastolic BP Peripheral" = dia_bp_periph, 
...
# AIx grouped by stove use
```{r}
summary_aix_stove_use <- r00_data_6phases %>% 
  filter(!is.na(assigned_stove)) %>% 
  filter(!is.na(aug_index)) %>% 
  select(stove_use_3, assigned_stove, aug_index, phase) %>% 
...
summary_aix_stove_use <- r00_data_5phases %>% 
  filter(!is.na(assigned_stove)) %>% 
  filter(!is.na(aug_index)) %>% 
  select(stove_use_3, assigned_stove, aug_index, phase) %>% 
  group_by(stove_use_3, phase) %>% 
...
# CPP grouped by stove use
```{r}
summary_cpp_stove_use <- r00_model_data_outliers_removed %>% 
  filter(!is.na(assigned_stove)) %>% 
  filter(!is.na(pulse_pressure_central)) %>% 
  select(stove_use_3, assigned_stove, pulse_pressure_central) %>% 
  group_by(stove_use_3) %>% 
  summarise("Mean" = mean(pulse_pressure_central),
...
kable(summary_cpp_stove_use, digits = 0)
```
